...
play :: Grid -> Player -> IO ()
play g p | wins O g = putStrLn "You win!\n"
         | wins X g = putStrLn "Computer wins!\n"
         | full g = putStrLn "It's a draw!\n"
         | p == O = do i <- getNat "Enter your move: "
                       case move g i p of
                         [] -> do putStrLn "ERROR: Invalid move"
                                  play g p
                         [g1] -> game g1 (next p)
         | p == X = do putStr "Computer is thinking... "
